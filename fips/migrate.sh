...
get_table_data_to_be_reencrypted() {
    ...
    local psql_command="SELECT json_agg(t) FROM (SELECT ${id_column}, ${encrypted_column}, ${salt_column} FROM ${table_name} WHERE ${encrypted_column} IS NOT NULL AND LENGTH(${salt_column}) BETWEEN 1 AND 15) t"
